...
# from model_cls import SwinTransformer
...
def train(args, train_loader, encoder, decoder, criterion, encoder_optimizer,encoder_lr_scheduler, decoder_optimizer, decoder_lr_scheduler, epoch, save_log_path='./data/logs.txt'):
    """
    ...
    for i, (imgs, caps, caplens) in enumerate(train_loader):
        ...
        with open(save_log_path, mode='a') as f:
            f.write("Epoch: {}/{} step: {}/{} Loss: {} AVG_Loss: {} Top-5 Accuracy: {} Batch_time: {}s".format(epoch+0, args.epochs, i+0, len(train_loader), losses.val, losses.avg, top5accs.val, batch_time.val))
            f.write('\n')
            f.close()

def validate(args, val_loader, encoder, decoder, criterion, save_log_path='./data/logs.txt'):
    """
    ...
    print('\n')
    with open(save_log_path, mode='a') as f:
        f.write("EVA LOSS: {} TOP-5 Accuracy {} BLEU-1 {} BLEU2 {} BLEU3 {} BLEU-4 {} METEOR {} ROUGE_L {} CIDEr {}".format(
            losses.avg, top5accs.avg,  metrics["Bleu_1"],  metrics["Bleu_2"],  metrics["Bleu_3"],  metrics["Bleu_4"],
            metrics["METEOR"],metrics["ROUGE_L"], metrics["CIDEr"]))
        f.write('\n')
        f.close()
...
if __name__ == '__main__':
    ...
    # torch.cuda.set_per_process_memory_fraction(0.25, device=1)
    DATASET = 'UCM_captions'  # RSICD / UCM_captions / Sydney_captions
    parser.add_argument('--train_name', default='checkpoint_'+DATASET+'_swint3', help='saving name of current training')

    # Data parameters
    parser.add_argument('--data_folder', default="/home/wyn/myStudy/2401-RSIC/datasets/"+DATASET+"/processed_with_name/", help='folder with data files saved by create_input_files.py.')
    parser.add_argument('--data_name', default="_5_cap_per_img_5_min_word_freq", help='base name shared by data files.')
    ...
    parser.add_argument('--encoder_mode', default="swin", help='which model does encoder use?') # inception_v3 or vgg16 or vgg19 or resnet50 or resnet101 or resnet152 or swin
    ...
    parser.add_argument('--workers', type=int, default=28, help='for data-loading; right now, only 0 works with h5pys in windows.')
    parser.add_argument('--encoder_lr', type=float, default=1e-4, help='learning rate for encoder if fine-tuning.')
    parser.add_argument('--decoder_lr', type=float, default=1e-4, help='learning rate for decoder.')        # default=1e-4
    ...
    parser.add_argument('--fine_tune_embedding', type=bool, default=False, help='whether fine-tune word embeddings or not')     # default=False
    parser.add_argument('--checkpoint', default=None, help='path to checkpoint, None if none.')
    parser.add_argument('--embedding_path', default=None, help='path to pre-trained word Embedding.')


    args = parser.parse_args()
    if not os.path.exists('/data_m1/tmp_data/2401-RSIC/CMR/' + args.train_name):
        os.makedirs('/data_m1/tmp_data/2401-RSIC/CMR/' + args.train_name)
    ...
    for encoder_layers, decoder_layers in [(3, 3)]: #,,(0,6),(2,2),
        ...
        if args.checkpoint is None:
            ...
            # Encoder
            # if args.encoder_mode == 'SwinT':
            #     encoder = SwinT_encoder(NetType=args.encoder_mode, attention_method=args.attention_method)
            #     encoder_optimizer = build_optimizer(encoder)
            #     encoder_lr_scheduler = StepLR(encoder_optimizer, step_size=4, gamma=0.9)
            #     encoder_dim = 2048
            if args.encoder_mode == 'swin':
                encoder = Swin_Encoder(NetType=args.encoder_mode, attention_method=args.attention_method)
                encoder.fine_tune(args.fine_tune_encoder)
                # optimizer = optim.AdamW(parameters, eps=1e-08, betas=(0.9, 0.999),
                #                         lr=0.000125, weight_decay=0.05)
                encoder_optimizer = build_optimizer(encoder)
                encoder_lr_scheduler = StepLR(encoder_optimizer, step_size=4, gamma=0.9)
                encoder_dim = 2048
            else:
                ...
                encoder_lr_scheduler = StepLR(encoder_optimizer, step_size=4, gamma=0.9)    # 600
                # set the encoder_dim
                encoder_dim = 512 if args.encoder_mode == 'vgg16' else 512 if args.encoder_mode == 'vgg19' else 2048  # FIXME: encoder_dim depends on the model
        ...
        # Epochs
        log_txt_path = '/data_m1/tmp_data/2401-RSIC/CMR/' + args.train_name + '/logs_'+DATASET+str(time.time())+'.txt'
        for epoch in range(start_epoch, args.epochs):

            # Decay learning rate if there is no improvement for 5 consecutive epochs, and terminate training after 25
            # 8 20
            # if epochs_since_improvement >= args.stop_criteria and metrics["Bleu_1"] > 0.80:
            ...
            # Save checkpoint
            checkpoint_name = args.encoder_mode + '_' + args.decoder_mode + '_' + str(args.encoder_layers) + '_' + str(args.decoder_layers) + '_Res+CMR+fusion' #_tengxun_aggregation
            ...
            save_checkpoint(checkpoint_name, epoch, epochs_since_improvement, encoder, decoder, encoder_optimizer,
                            ...
                            save_path='/data_m1/tmp_data/2401-RSIC/CMR/'+args.train_name)
